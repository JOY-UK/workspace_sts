...
//바로구매 버튼 클릭 시 실행
function buyy(itemCode){
   const buyCnt = parseInt(document.querySelector('#cartCnt').value)
   const regex = /[^0-9]/g;
   const totalPrice = parseInt(document.querySelector('#totalPrice').textContent.replace(regex,''))
   
   location.href = `/buy/oneBuy?itemCode=${itemCode}&totalPrice=${totalPrice}&buyCnt=${buyCnt}`
}